...
// interface for FibreLanding page cms content: TODO: move to models
export interface IFibreContent {
...
async function getFibreLandingPageContent(): Promise<IFibreContent[]> {
    let fibreLandingPageContent: IFibreContent[] = [];
    ...
    let graphql = JSON.stringify({
        query: `query FibreLandingPage2023 {
                    entries(
                        section: "connectivityLandingPages"
                        slug: "fibre-2023-landing-page"
                        limit: 1
                        orderBy: "dateCreated DESC"
                    ) {
                        title
                        slug
                        ... on connectivityLandingPages_fibre2023_Entry {
                            __typename
                            seoPageTitle
                            seoPageDescription
                            simpleBanner {
                            ...
                            vasFeatureDescription {
                                ... on vasFeatureDescription_featureGroupHeading_BlockType {
                                    __typename
                                    heading
                                }
                                ... on vasFeatureDescription_features_BlockType {
                                    __typename
                                    featureIcon
                                    featureName
                                    featureDescription
    ...
    try {
        ...
        if (result?.data?.entries?.length > 0) {
            fibreLandingPageContent = result?.data?.entries;
        }
    } catch (error) {
        console.log(
            `\n---\n failed to get fibreLandingPage content from craft due to: \n ${error} \n`
        );
    }

    return fibreLandingPageContent;
}
export { getFibreLandingPageContent };
